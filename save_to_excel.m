...
load(uigetfile('*.mat'));
Mixed_cycles=false; % change to true in case you want to see results of the opposite side 
...
joint_names=string(fieldnames(xsens_preprocessed));
joint_names(1:2,:)=[];


All_res_RC=[];
for j=1:length(joint_names)
    if (Mixed_cycles==false && contains(lower(joint_names(j)),'left'))
         disp(strcat(joint_names(j)," skipped during right cycles"));
    else
...
All_res_LC=[];
for j=1:length(joint_names)
    if (Mixed_cycles==false && contains(lower(joint_names(j)),'right'))
         disp(strcat(joint_names(j)," skipped during left cycles"));
    else
        ROM_temp=[];MAX_temp=[];MIN_temp=[];
...
for m=1:length(muscle_names)
    if (Mixed_cycles==false && contains(lower(muscle_names(m)),'left'))
         disp(strcat(muscle_names(m)," skipped during right cycles"));
    else
        ENV_temp=[]; 
        for c=1:xsens_processed.cycles(1) % Right cycles
            eval(strcat("ENV_temp=[ENV_temp;mean(emg_param.",muscle_names(m),".RC.env(c,:))];"))
...
All_res_LC=[];
for m=1:length(muscle_names)
    if (Mixed_cycles==false && contains(lower(muscle_names(m)),'right'))
         disp(strcat(muscle_names(m)," skipped during left cycles"));
    else
        ENV_temp=[]; Peak_temp=[]; RMS_temp=[];
        for c=1:xsens_processed.cycles(2) % Left cycles
            eval(strcat("ENV_temp=[ENV_temp;mean(emg_param.",muscle_names(m),".LC.env(c,:))];"))
...
if (Mixed_cycles==true) 
    ...
    temp=contains(lower(joint_names),'right');
    writetable([array2table(joint_names(find(temp==1)),...
        'VariableNames',{'Joint Name'}) tbl_kinematics_rc], OutFilename,...
        'Sheet', 'Right Cycles','Range','A1');  
    writetable([array2table(joint_names(find(temp==0)),...
        'VariableNames',{'Joint Name'}) tbl_kinematics_lc], OutFilename,...
        'Sheet', 'Left Cycles','Range','A1');
    temp=contains(lower(muscle_names),'right');
    writetable([array2table(muscle_names(find(temp==1)),...
        'VariableNames',{'Muscle Name'}) tbl_EMG_rc], OutFilename,...
        'Sheet', 'Right Cycles','Range','A15');
    writetable([array2table(muscle_names(find(temp==0)),...
        'VariableNames',{'Muscle Name'}) tbl_EMG_lc], OutFilename,...
        'Sheet', 'Left Cycles','Range','A15');
